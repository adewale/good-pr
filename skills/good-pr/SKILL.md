...
and run the installed checker:

```bash
python3 scripts/check-visual-evidence.py --kind ui /tmp/pr-body.md
python3 scripts/check-visual-evidence.py --kind generated /tmp/pr-body.md
```
...
volume, and never downloads or judges the pixels. See
